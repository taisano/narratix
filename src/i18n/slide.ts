...
const SLIDE_TEXT = {
  ja: {
    ...
    yoy: '前年比',
    yoyRange: '前年比（{from}→{to}）',
    yoyShort: '前年比 {value}',
    totalChange: '合計：{from} → {to}（{detail}）',
    scopeFilter: '{name}：{value}',
    scopeShare: '{value} の割合（{name}の中）',
  ...
  en: {
    ...
    cagrShort: 'CAGR {value}',
    yoy: 'YoY',
    yoyRange: 'YoY ({from}→{to})',
    yoyShort: 'YoY {value}',
    totalChange: 'Total: {from} → {to} ({detail})',
    scopeFilter: '{name}: {value}',
    scopeShare: 'Share of {value} within {name}',
