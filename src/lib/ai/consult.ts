...
- TREND：時間とともにどう変わったか（推移、伸びてきた軌跡、成長の速さ）
- COMPARISON：項目どうしの大小・順位・差（予算と実績の差、前年と今年の項目ごとの差、ランキング）
- COMPOSITION：全体の中の割合・内訳・シェア、全体の規模とその中身
- CONTRIBUTION：ある数字の増減を要因（項目）に分けて、何がどれだけ効いたかを示す
...
- CLARIFY：何の数字か（指標）も、何を知りたいか（見方）も書かれていない時（例：「数字をまとめたい」）。
  または「平均より伸びている」のように、比べる基準（平均の取り方）で答えが変わる時
...
- MULTI_PERIOD：3時点以上の推移（「推移」「過去5年」「2021〜2025年」「月次」）
- TWO_POINT：2時点だけを比べる（前年と今年、前年比、2021年と2025年、開始と終了）
- UNKNOWN：どちらとも言えない
time_scope：期間が書かれていれば「2021-2025」「5y」の形。なければ null

comparison_intent（比較の意味）
- LEVEL：大小・順位（どこが一番大きいか）
- DELTA：差・増減（予算と実績の差、前年からの増減、どれだけ増えたか）
...
- BREAKDOWN：全体の推移とその内訳（何が全体の伸びを支えているか、内訳の推移）
- NONE／UNKNOWN

measure_additivity：ADDITIVE（売上・件数・人数・費用など足し合わせられる）、NON_ADDITIVE（率・平均・単価・指数・満足度など足せない）、UNKNOWN
...
- 「会員数の月次の推移と、その中の有料会員の割合の変化を報告したい」→ COMPOSITION、MULTI_PERIOD、composition_intent BREAKDOWN、audience REPORT
...
export function toClassification(a: ConsultAi): ConsultationClassification {
  ...
  return ConsultationClassificationSchema.parse({
    ...
    measure_additivity: a.measure_additivity,
