...
export function checkRecipeData(r: RecipeDef, dataset: Dataset): RecipeCheck {
  ...
  if (req.timeAxis) {
    const t = timeRange(dataset.rows);
    if (!t) err('needs_years');
      ...
      for (const [idx, year] of [[t.fromIndex, t.from], [t.toIndex, t.to]] as const) {
        ...
        if (missing.length) err('missing_endpoint', { year, cols: missing.join('、') });
      }
      if (r.derived.includes('cagr')) {
        const na = dataset.cols.filter((_, k) => { const v = vals[t.fromIndex]?.[k]; return v != null && v <= 0; });
        if (na.length) warn('cagr_na', { year: t.from, cols: na.join('、') });
      }
    }
  }
...
const TEXT: Record<RecipeIssueCode, LocalizedText> = {
  ...
  missing_endpoint: { ja: 'CAGR を計算するには、開始年と終了年の両方のデータが必要です。不足：{year}年の {cols}', en: 'CAGR needs data for both the start and end year. Missing: {cols} in {year}' },
  cagr_na: { ja: '{year}年の値が 0 以下のため、CAGR は計算できません（N/A と表示）：{cols}', en: 'CAGR cannot be calculated where the {year} value is 0 or less (shown as N/A): {cols}' },
