...
export const RECIPES: Record<RecipeId, RecipeDef> = {
  ...
  // ──────────── 要因 ────────────
  CONTRIB_WATERFALL: {
    fit: { time: ['NONE', 'TWO_POINT', 'MULTI_PERIOD'], comparison: ['DELTA'] },
    id: 'CONTRIB_WATERFALL', name: L('増減を要因に分解する', 'Break the change into drivers'),
    question: L('始点から終点まで、何がどれだけ効いたか', 'What moved the number from start to end, and by how much?'),
    goals: ['contribution'], composition: 'SINGLE_CHART', view: single('waterfall'),
    schema: 'DRIVER_BRIDGE', requirements: { minRows: 3 }, derived: ['difference', 'change_rate'],
    exactValues: true, readingLoad: 'low', audience: ['EXECUTIVE_MEETING', 'REPORT'],
    keywords: { ja: ['要因', '分解', 'ブリッジ', 'ウォーターフォール', '内訳', '寄与'], en: ['bridge', 'waterfall', 'drivers', 'breakdown'] },
    reason: L('始点から終点までの増減を、要因ごとの積み上げで順に見せます。どの要因でどれだけ増え、どれだけ減ったかが1本の流れで伝わります。', 'Walks from the start value to the end value driver by driver, so each increase and decrease reads as one flow.'),
    strength: L('増減の流れと、要因ごとの大きさが同時に分かる', 'Shows both the flow and the size of each driver'),
    limitation: L('要因が多いと細かくなり、どれが一番効いたかが埋もれる', 'With many drivers it gets busy and the biggest one can get lost'),
    priority: 9, status: 'ACTIVE',
  },
  CONTRIB_DRIVERS: {
    fit: { time: ['NONE', 'TWO_POINT', 'MULTI_PERIOD'], comparison: ['DELTA'] },
    id: 'CONTRIB_DRIVERS', name: L('一番効いた要因を見る', 'Which driver mattered most'),
    question: L('どの要因が一番効いたか', 'Which driver had the biggest impact?'),
    goals: ['contribution'], composition: 'SINGLE_CHART', view: single('driver_bar'),
    schema: 'DRIVER_BRIDGE', requirements: { minRows: 3 }, derived: ['difference', 'rank'],
    exactValues: true, readingLoad: 'low', audience: ['EXECUTIVE_MEETING', 'SALES_MEETING', 'REPORT'],
    keywords: { ja: ['一番効いた', '大きな要因', '主因', '影響の大きい'], en: ['biggest driver', 'main driver'] },
    reason: L('要因だけを取り出して、影響の大きい順に並べます。増えた要因は右、減った要因は左に伸びます。', 'Lists only the drivers, largest impact first; increases extend right and decreases left.'),
    strength: L('要因の大小と向きがすぐ分かる', 'Size and direction of each driver are clear at once'),
    limitation: L('始点・終点の水準は見えない', 'The start and end levels are not shown'),
    priority: 7, status: 'ACTIVE',
  },
  CONTRIB_POSNEG: {
    fit: { time: ['NONE', 'TWO_POINT', 'MULTI_PERIOD'], comparison: ['DELTA'] },
    id: 'CONTRIB_POSNEG', name: L('増加要因と減少要因を分けて見る', 'Positive and negative drivers apart'),
    question: L('何が押し上げ、何が押し下げたか', 'What pushed the number up, and what pulled it down?'),
    goals: ['contribution'], composition: 'SINGLE_CHART', view: single('posneg_bar'),
    schema: 'DRIVER_BRIDGE', requirements: { minRows: 3 }, derived: ['difference'],
    exactValues: true, readingLoad: 'low', audience: ['EXECUTIVE_MEETING', 'REPORT'],
    keywords: { ja: ['押し上げ', '押し下げ', 'プラス要因', 'マイナス要因', '増加要因', '減少要因'], en: ['tailwinds', 'headwinds', 'positive and negative'] },
    reason: L('増加要因と減少要因を左右に分け、同じ尺度で大きさを並べます。追い風と向かい風を分けて説明したい時に向きます。', 'Splits increases and decreases into two panels on one scale, for explaining tailwinds and headwinds separately.'),
    strength: L('プラスとマイナスの顔ぶれがはっきり分かれる', 'Positives and negatives are clearly separated'),
    limitation: L('差し引きの結果（純増減）は見えにくい', 'The net result is harder to see'),
    priority: 6, status: 'ACTIVE',
  },

  // ──────────── 関係 ────────────
  REL_SCATTER: {
    fit: { time: ['NONE', 'TWO_POINT', 'MULTI_PERIOD'] },
    id: 'REL_SCATTER', name: L('2つの指標の関係を見る', 'How two measures relate'),
    question: L('一方が大きいほど、もう一方も大きいか', 'When one is higher, is the other higher too?'),
    goals: ['relationship'], composition: 'SINGLE_CHART', view: single('scatter'),
    schema: 'BUBBLE', requirements: { minRows: 3 }, derived: [],
    exactValues: false, readingLoad: 'medium', audience: ['EXECUTIVE_MEETING', 'REPORT'],
    keywords: { ja: ['関係', '相関', '比例', 'ほど'], en: ['relationship', 'correlation'] },
    reason: L('項目ごとに2つの指標を縦と横に取り、点の並びで関係を見せます。相関係数も添えます。', 'Plots each item on two measures so the pattern shows the relationship, with the correlation noted.'),
    strength: L('関係の向きと強さ、外れた項目が分かる', 'Shows direction and strength, and the outliers'),
    limitation: L('項目ごとの規模は見えない。相関は因果を示さない', 'Size is not shown, and correlation is not causation'),
    priority: 8, status: 'ACTIVE',
  },
  REL_QUADRANT: {
    fit: { time: ['NONE', 'TWO_POINT', 'MULTI_PERIOD'] },
    id: 'REL_QUADRANT', name: L('4つに分けて位置づける', 'Place items in four quadrants'),
    question: L('どの項目が、どの位置（象限）にいるか', 'Which items sit in which quadrant?'),
    goals: ['relationship'], composition: 'SINGLE_CHART', view: single('scatter', { inChartComplements: [{ id: 'quadrants' }] }),
    schema: 'BUBBLE', requirements: { minRows: 4 }, derived: [],
    exactValues: false, readingLoad: 'medium', audience: ['EXECUTIVE_MEETING', 'SALES_MEETING', 'REPORT'],
    keywords: { ja: ['ポジショニング', '象限', 'マトリクス', '位置づけ', '分類'], en: ['positioning', 'quadrant', 'matrix'] },
    reason: L('散布図を2つの指標の中央値で4つに分け、項目がどの位置にいるかを見せます。', 'Splits the scatter at the median of each measure so each item lands in a quadrant.'),
    strength: L('項目の位置づけ（強み・弱み）が一目で分かる', 'Positioning of each item is clear at a glance'),
    limitation: L('境目は中央値なので、境目近くの項目の区分は揺れやすい', 'The split is the median, so items near it can flip'),
    priority: 6, status: 'ACTIVE',
  },
  REL_BUBBLE: {
    fit: { time: ['NONE', 'TWO_POINT', 'MULTI_PERIOD'] },
    id: 'REL_BUBBLE', name: L('関係と規模を1枚で', 'Relationship and size together'),
    question: L('関係の中で、規模の大きい項目はどこにいるか', 'Where do the big items sit in the relationship?'),
    goals: ['relationship'], composition: 'SINGLE_CHART', view: single('bubble'),
    schema: 'BUBBLE', requirements: { minRows: 3 }, derived: [],
    exactValues: false, readingLoad: 'medium', audience: ['EXECUTIVE_MEETING', 'REPORT'],
    keywords: { ja: ['規模', '大きさ', 'バブル', 'ポートフォリオ'], en: ['bubble', 'portfolio', 'size'] },
